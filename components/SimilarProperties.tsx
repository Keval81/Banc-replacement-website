    ...
    <div className={`${className}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <a 
          ...
          className="text-sm text-[#1DBFDD] hover:underline flex items-center gap-1"
        >
          View all
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {properties.map((property, index) => (
          ...
          >
            ...
            {/* Content */}
            <div className="p-4">
              <h3 className="font-semibold text-gray-900 truncate group-hover:text-[#1DBFDD] transition-colors">
                {property.title}
              </h3>
              <p className="text-sm text-gray-500 truncate mb-2">{property.address}</p>
              
              <p className="text-lg font-bold text-[#0D9488] mb-3">{property.price}</p>
