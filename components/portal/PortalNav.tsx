...
export default function PortalNav({ userRole, onNavigate }: PortalNavProps) {
  ...
  return (
    <div className="flex flex-col h-full py-4">
            ...
            <a
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                active
                  ...
                  : "text-[#6B6E72] hover:bg-[#F0F0ED] hover:text-[#2C2F33]"
              }`}
            >
              ...
              {item.badge && (
                <span
                  className={`px-2 py-0.5 text-xs rounded-full ${
                    active ? "bg-white/20 text-white" : "bg-[#1DBFDD] text-white"
                  }`}
                >
                  {item.badge}
                </span>
      ...
      <div className="px-3 pt-4 border-t border-[#C8C9CB] space-y-1">
      ...
      {/* Help Card */}
      <div className="px-4 py-4 mt-4 mx-3 bg-[#F0F0ED] rounded-xl">
        <p className="text-xs text-[#6B6E72] mb-2">Need assistance?</p>
        <a
          href="/contact"
