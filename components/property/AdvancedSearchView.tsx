...
function OptionList<TValue extends string>({ name, options, selected, canonicalOrder, onChange }: {
  ...
  return <div className="space-y-1">{options.map((option) => {
    ...
    return (
      <label key={option.value} htmlFor={id} className={cn("flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border p-3 transition-colors duration-200", checked ? "border-[#4AC8E8]/30 bg-[#4AC8E8]/5 text-[#1A1917]" : "border-transparent text-[#5F5D57] hover:bg-[#F4F3F1]")}>
    ...
    <div className={cn("flex h-full min-w-0 flex-col bg-white", !isMobile && "rounded-2xl border border-[#E0DFDC] shadow-sm")}>
      <header className={cn("flex items-center justify-between gap-3 border-b border-[#E0DFDC] px-5 py-4", isMobile && "sticky top-0 z-10 bg-white")}>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {isMobile && onClose && <button type="button" onClick={onClose} className="flex h-11 w-11 items-center justify-center rounded-full transition-colors duration-200 hover:bg-[#F4F3F1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4AC8E8]" aria-label="Close filters"><X className="h-5 w-5 text-[#5F5D57]" /></button>}
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-5">
        ...
        <FilterSection title="Location" icon={MapPin}>
          ...
          <div className="relative"><MapPin className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5F5D57]" /><Input id="advanced-location" type="text" placeholder="Enter area, town or postcode" value={filters.location ?? ""} onChange={(event) => onFilterChange({ location: event.target.value || undefined })} className="h-12 bg-[#F4F3F1] pl-10 pr-12 text-base placeholder:text-[#5F5D57] focus:bg-white" />{filters.location && <button type="button" onClick={() => onFilterChange({ location: undefined })} className="absolute right-0 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4AC8E8]" aria-label="Clear location"><X className="h-4 w-4 text-[#5F5D57]" /></button>}</div>
        </FilterSection>
