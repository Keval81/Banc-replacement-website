      ...
      <section className="relative bg-[#2C2F33] py-24 lg:py-32 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-6 lg:px-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-6">
            <HelpCircle className="h-4 w-4 text-[#1DBFDD]" />
          ...
          <div className="mt-10 max-w-2xl mx-auto">
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-[#6B6E72]" />
              <Input 
                placeholder="Search for answers..."
                className="h-14 pl-12 pr-4 rounded-xl border-0 bg-white text-[#2C2F33] placeholder:text-[#6B6E72] focus:ring-2 focus:ring-[#1DBFDD]"
              />
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="hidden lg:block">
              <div className="sticky top-32">
                <h3 className="text-sm font-semibold uppercase tracking-wider text-[#6B6E72] mb-4">
                  Categories
                </h3>
                <nav className="space-y-1">
                  {faqCategories.map((category) => (
                    <a
                      key={category.id}
                      href={`#${category.id}`}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#3A3D42] hover:bg-white hover:shadow-sm transition-all group"
                    >
                      <category.icon className="h-5 w-5 text-[#6B6E72] group-hover:text-[#1DBFDD] transition-colors" />
                ...
                <div key={category.id} id={category.id}>
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-12 h-12 rounded-xl bg-[#1DBFDD]/10 flex items-center justify-center">
                      <category.icon className="h-6 w-6 text-[#1DBFDD]" />
                    </div>
                    <h2 className="text-2xl font-semibold text-[#2C2F33]">{category.name}</h2>
                  </div>
      ...
      <section className="py-20 lg:py-28 bg-[#2C2F33]">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-6">
              <MessageCircle className="h-4 w-4 text-[#1DBFDD]" />
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {/* Phone */}
            <a href="tel:01707877781" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center hover:border-[#1DBFDD]/50 transition-colors group">
              <div className="w-14 h-14 rounded-xl bg-[#1DBFDD]/20 flex items-center justify-center mx-auto mb-4">
                <Phone className="h-7 w-7 text-[#1DBFDD]" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Call Us</h3>
              <p className="text-white/60 text-sm mb-3">Speak to our team directly</p>
              <p className="text-[#1DBFDD] font-semibold group-hover:underline">01707 877781</p>
            </a>
            
            {/* Email */}
            <a href="mailto:info@bancproperty.com" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center hover:border-[#1DBFDD]/50 transition-colors group">
              <div className="w-14 h-14 rounded-xl bg-[#1DBFDD]/20 flex items-center justify-center mx-auto mb-4">
                <MessageCircle className="h-7 w-7 text-[#1DBFDD]" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Email Us</h3>
              <p className="text-white/60 text-sm mb-3">We'll respond within 24 hours</p>
              <p className="text-[#1DBFDD] font-semibold group-hover:underline">info@bancproperty.com</p>
            </a>
            
            {/* Visit */}
            <Link href="/contact" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center hover:border-[#1DBFDD]/50 transition-colors group">
              <div className="w-14 h-14 rounded-xl bg-[#1DBFDD]/20 flex items-center justify-center mx-auto mb-4">
                <Building className="h-7 w-7 text-[#1DBFDD]" />
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">Visit Us</h3>
              <p className="text-white/60 text-sm mb-3">Come and see us in person</p>
              <p className="text-[#1DBFDD] font-semibold group-hover:underline">Find Our Office</p>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="text-center mb-12">
            <h2 className="text-2xl font-semibold text-[#2C2F33]">Popular Resources</h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <Link 
                key={item.title}
                href={item.link}
                className="flex items-center gap-4 p-4 bg-white rounded-xl border border-[#C8C9CB]/30 hover:border-[#1DBFDD]/50 hover:shadow-md transition-all group"
              >
                <div className="w-12 h-12 rounded-lg bg-[#1DBFDD]/10 flex items-center justify-center flex-shrink-0">
                  <item.icon className="h-5 w-5 text-[#1DBFDD]" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-[#2C2F33] group-hover:text-[#1DBFDD] transition-colors">
                    {item.title}
                  </h3>
                  <p className="text-sm text-[#6B6E72]">{item.desc}</p>
                </div>
                <ArrowRight className="h-5 w-5 text-[#C8C9CB] group-hover:text-[#1DBFDD] group-hover:translate-x-1 transition-all" />
              </Link>
            ))}
          </div>
        </div>
