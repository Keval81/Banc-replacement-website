    ...
    <div className="bg-white text-[#111827]">
      ...
      <section className="relative bg-[#2C2F33] py-24 lg:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="max-w-3xl">
            <p className="text-sm uppercase tracking-[0.3em] text-[#1DBFDD] mb-4">Results</p>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center p-6 rounded-2xl border border-[#E5E7EB]">
                <stat.icon className="h-10 w-10 text-[#1DBFDD] mx-auto" />
                <p className="mt-4 text-4xl font-bold text-[#1DBFDD]">{stat.value}</p>
      ...
      <section className="bg-[#F9FAFB] py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="text-center mb-12">
            <p className="text-sm uppercase tracking-[0.3em] text-[#1DBFDD]">Recent Success</p>
            <h2 className="mt-4 text-3xl font-semibold sm:text-4xl">Recent Sales</h2>
          </div>
          
          <div className="grid gap-6 md:grid-cols-2">
            {recentSales.map((sale) => (
              <div key={sale.address} className="rounded-2xl border border-[#E5E7EB] bg-white p-6">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-lg font-semibold">{sale.address}</p>
                    <div className="mt-2 flex items-center gap-4">
                      <span className="text-2xl font-bold text-[#1DBFDD]">{sale.price}</span>
                      {sale.price !== sale.originalPrice && (
                        <span className="text-sm text-[#6B7280] line-through">{sale.originalPrice}</span>
                      )}
                    </div>
                  </div>
          ...
          <div className="mt-8 text-center">
            <Button variant="outline" className="border-[#1DBFDD] text-[#1DBFDD] hover:bg-[#1DBFDD] hover:text-white">
      ...
      <section className="py-20">
        <div className="mx-auto max-w-4xl px-6 lg:px-10 text-center">
          <div className="text-6xl text-[#1DBFDD] opacity-30">"</div>
          <blockquote className="text-2xl font-medium text-[#111827] -mt-8">
          ...
          <div className="mt-6">
            ...
            <p className="text-[#6B7280]">Hadley Wood</p>
          </div>
        </div>
      </section>
