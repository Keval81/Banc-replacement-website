...
test("repairs malformed intent exactly once with validation feedback", async () => {
  ...
  assert.match(fetch.calls[1]?.body ?? "", /validation/i);
...
test("writes one grounded response using only sanitized results", async () => {
  const fetch = createSequenceFetch([
    openAIJsonResponse({ response: "I found one five-bedroom home in Cuffley." }),
  ]);
  const model = createOpenAIConversationModel({
    apiKey: "test-key",
    model: "gpt-test",
  ...
  assert.deepEqual(result, {
    status: "ok",
    response: "I found one five-bedroom home in Cuffley.",
    providerCalls: 1,
  });
  assert.equal(fetch.calls.length, 1);
